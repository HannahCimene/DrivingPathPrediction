...
dfvs = np.column_stack((dfv['lat'].values, dfv['lon'].values))

# mean_lat = dfv['lat'].mean()
# centered_lat = dfv['lat'].values - mean_lat

# # Use centered_lat when creating your sequences (slat in the previous code)
# dfvs = np.column_stack((centered_lat, dfv['lon'].values)) # Or center longitude as well
...
plt.plot(ylat, label='real')
plt.plot(ylat_pred, label='predict')
plt.title(label='Predicted lat with LSTM (Unscaled)')
