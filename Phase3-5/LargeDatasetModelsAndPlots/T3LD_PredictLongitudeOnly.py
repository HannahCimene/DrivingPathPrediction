...
def transform_taxi_data(df):
    ...
    df = df[['vid', 'ts', 'lon']].copy()  # Keep only 'lon'
...
def build_lstm_model_with_attention_lon_only(input_shape):
    """Builds a simplified LSTM model with attention for longitude prediction."""
...
### RNN Model
rnn_model_lon = build_simple_rnn_model_lon_only(input_shape)
...
### LSTM Model
# lstm_model_lon = build_lstm_model_lon_only(input_shape)
...
# lstm_attention_model_lon = build_lstm_model_with_attention_lon_only(input_shape)
...
model_to_evaluate = trained_rnn_model_lon
...
if len(y_test_np) == len(y_pred_np):
    print("\nFirst 20 Actual vs. Predicted Values (Longitude):")
...
y_predicted = evaluate_and_plot_model(model_to_evaluate, X_test, y_test, sequence_length, f"{model_name} (Longitude)")

### Visualizing Predictions Against Actual Values (More Detailed):
plot_predictions(y_test, y_predicted, f"{model_name} (Longitude)", n_samples)

### Plotting the residuals
plot_residuals(y_test, y_predicted, f"{model_name} (Longitude)")

### Predictions with actual sequences
# Overal model

# Specific vehicle
plot_single_vehicle_predictions(model_to_evaluate, transformed_df, vehicle_id, sequence_length, coordinate_column='lon')

### Autoregressive predictions of a specific vehicle
plot_actual_vs_predicted_with_future(model_to_evaluate, transformed_df, vehicle_id, sequence_length, coordinate_column='lon', n_predictions=100)
